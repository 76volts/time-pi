...
MENU_ITEMS = [
    ...
    ("Pomodoro", "screens/pomodoro_screen.py"),
]
...
def main():
    ...
    accumulated = 0
    scroll_threshold = 2
    last_backlight_check = time.time()
    BACKLIGHT_CHECK_INTERVAL = 10  # Check every 10 seconds

    render_menu(lcd, MENU_ITEMS, selected)

    try:
        while True:
            # --- Night Mode Control (every 10 seconds) ---
            now = time.time()
            if now - last_backlight_check >= BACKLIGHT_CHECK_INTERVAL:
                lcd.backlight(not is_night_time())
